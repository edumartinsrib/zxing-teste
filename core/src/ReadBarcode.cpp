...
{
	char* cppString = "Isso é uma string em C++";

	return cppString;

	// printf("%s\n buffer",buffer);

	// ImageView iv(buffer, width, height, static_cast<ImageFormat>(0x03000102));

	// std::cout << "passou aqui" << buffer;

	// auto autoHints = DecodeHints().setTryHarder(false).setTryRotate(false).setIsPure(false);

	// Results results = ReadBarcodes(iv, autoHints);

	// std::cout << "results passou aqui 3" + results[0].text() << results[0].text();
...
Results ReadBarcodes(const ImageView& _iv, const DecodeHints& hints)
{
	...
	auto formatsBenefittingFromClosing =
		BarcodeFormat::Aztec | BarcodeFormat::DataMatrix | BarcodeFormat::QRCode | BarcodeFormat::MicroQRCode;
